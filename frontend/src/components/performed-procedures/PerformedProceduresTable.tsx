...
interface PerformedProceduresTableProps {
  ...
  onDeleteProcedure: (p: PerformedProcedure) => void;
  sortColumn?: PerformedProcedureSortBy | null;
  sortDirection?: PerformedProcedureSortOrder;
...
const PerformedProceduresTable = ({
  ...
  const actionButtons = (p: PerformedProcedure) => (
    <div className="flex items-center justify-center gap-1.5" onClick={(e) => e.stopPropagation()}>
      <button
        type="button"
        title="Ver detalle"
        onClick={() => onViewProcedure(p)}
  ...
  const columns: Column<PerformedProcedure>[] = [
    ...
    },
    {
      key: 'actions',
      header: 'Acciones',
      className: 'text-center w-[12.5rem]',
      hideOnMobile: true,
        ...
        {loading ? (
          ...
          data.map((p) => (
            ...
            >
              ...
              <div className="flex justify-center pt-3 mt-2 border-t border-slate-100 dark:border-slate-700">
                {actionButtons(p)}
              </div>
            </div>
          ))
